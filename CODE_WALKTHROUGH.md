...
输出一：多帧对齐的 OGM 序列 → 喂给 ConvLSTM κ(·)

通过 discretize() 将每帧激光转成二值 OGM

经过自运动补偿后，这 10 帧 OGM 都在同一个未来坐标系 R 下

直接作为 ConvLSTM 的输入 O^R_{t-τ:t}

输出二：局部静态环境地图 m → 喂给 VAE

update() 方法用贝叶斯滤波累积多帧观测

动态物体（行人）因为位置不固定，在累积过程中被当作噪声"平均掉"了

静态物体（墙壁）在每帧的位置不变，概率不断累加

最终 retrieve_p() 输出的概率图就是静态地图 m
```
